...
export default function LoginPage({ mode = 'signin' }: { mode?: 'signin' | 'signup' }) {
  ...
  const handleContinue = async (e: React.FormEvent) => {
    ...
    try {
      if (isFirebaseEnabled) {
        setError('Email sign in is only available in local development mock mode. Please use Google Sign In.');
        setLoading(false);
      } else {
        // Mock Auth Flow
        ...
        toast.success(mode === 'signin' ? `Logged in as ${trimmedEmail}` : `Signed up with ${trimmedEmail}`);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      if (isFirebaseEnabled) {
        // Calls standard Google sign in via popup redirect
        await loginWithRedirect({ redirectUrl });
      } else {
        // Mock Google sign in
        const mockAdminEmail = 'admin@prabhupadaworld.org';
        localStorage.setItem('auth_email', mockAdminEmail);
        (window as any).__firebase_id_token = `mock_token_for_${mockAdminEmail}`;
        
        toast.success(`Google Sign In: Logged in as Admin (${mockAdminEmail})`);
  ...
  return (
    <div className="min-h-screen w-full bg-[#fefdfa] flex items-center justify-center p-4">
      <div className="w-full max-w-[420px] bg-white border border-gray-200/80 rounded-2xl shadow-[0_10px_35px_rgba(0,0,0,0.08),_0_1px_6px_rgba(0,0,0,0.03)] p-8 md:p-10 text-center">
        ...
        <button
          ...
          disabled={loading}
          className="w-full h-11 bg-[#fffaf0] hover:bg-[#fef3d6] border border-[#fbe9c3] text-gray-800 font-semibold text-sm rounded-lg transition-all flex items-center justify-center gap-3 cursor-pointer shadow-2xs hover:shadow-xs disabled:opacity-70 disabled:cursor-not-allowed"
        >
          {/* Google Logo G Icon */}
          ...
          {mode === 'signin' ? (
            ...
            <>
              ...
              <span 
                ...
                className="text-gray-955 hover:text-black font-semibold ml-1 cursor-pointer transition-colors"
              >
                Sign in
              </span>
            </>
